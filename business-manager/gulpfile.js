(function() {
	...
	gulp.task('inlineAngularTemplates', function() {
		return gulp.src(['src/*/**/*.html', '!src/widgets/summernote/examples/textarea.html', '!src/widgets/map/baidu_map.html'])
	...
	gulp.task('useref', function() {
		return gulp.src(['src/index.html', 'src/login.html'])
			.pipe(useref())
	...
	gulp.task('revCollector', function() {
		return gulp.src(['tmp/rev/*.json', 'tmp/index.html', 'tmp/login.html']) //- 读取 rev-manifest.json 文件以及需要进行css名替换的文件
			.pipe(revCollector()) //- 执行文件内css名的替换
			.pipe(gulp.dest('build/')); //- 替换后的文件输出的目录
	});

	//copy文件
	gulp.task('copy', ['copyImg', 'copySvg', 'copyhtml', 'copyMap', 'copySummernote' ,'copyCity'])
	...
	gulp.task('copyMap', function() {
		return gulp.src(['src/widgets/map/*.*'])
			.pipe(gulp.dest("build/widgets/map"));
	})

	gulp.task('copySummernote', function() {
		return gulp.src(['src/widgets/summernote/**/*.*'])
			.pipe(gulp.dest("build/widgets/summernote"));
	})
	...
	gulp.task('templateCache', function() {
		return gulp.src(['src/*/**/*.html', '!src/bower_components/**/*.html',  '!src/widgets/summernote/examples/textarea.html', '!src/widgets/map/baidu_map.html'])
			.pipe(templateCache('templates.js', {
				module: 'businessManagerApp',
				root: ''
			}))
	...
	//开发watch引入模板临时文件templates.js
	gulp.task('loadTemplateCache', function() {
		return gulp.src('src/index.html')
			.pipe(inject(gulp.src('src/templates.js', {
				read: false
			}), {
				relative: true
			}))
	...
	gulp.task('cdn', function() {
		gulp.src('build/*.html')
			.pipe(cdn({
				dir: './build',
				root: {
					js: 'http://static-1.520dyw.cn',
					css: 'http://static-1.520dyw.cn'
				}
			}))
			.pipe(gulp.dest('./build'));
	});
